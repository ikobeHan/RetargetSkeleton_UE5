...
void FRetargetSkeletonModule::ExtendSkeletonBrowserMenu()
{
	...
	for (const FName& MenuName : MenusToExtend)
	{
		...
		Section.AddSubMenu(
			...
			LOCTEXT("RetargetSkeltonSubmenu", "Retarget Skeleton"),
...
void FRetargetSkeletonModule::CreateRetargetSubMenu(FToolMenuSection& InSection)
{
	...
	InSection.AddMenuEntry(
		...
		LOCTEXT("Skeleton_Retarget_ue5", "Retarget ALL-REF-ASSETS to Another Skeleton"),
		LOCTEXT("Skeleton_Retarget_ue5Tooltip", "通过 IKRetargeter 重定向骨架关联的全部动画（需要先建立好Retargeter文件）"),
...
IMPLEMENT_MODULE(FRetargetSkeletonModule, RetargetSkeleton)